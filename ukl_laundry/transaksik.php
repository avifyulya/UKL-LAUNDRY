...
<table class="table table-hover table-striped">
    ...
    <tbody>
        <?php 
        include "connect.php";
        $qry_transaksi=mysqli_query($connect,"SELECT * FROM tampil_paket order by id_paket desc");
        $no=0;
        while($dt_transaksi=mysqli_fetch_array($qry_transaksi)){
            $no++;
            $buku_dipinjam="<ol>";
            $qry_buku=mysqli_query($connect,"select * from  detail_transaksi join buku on buku.id_buku=detail_peminjaman_buku.id_buku where id_peminjaman_buku = '".$dt_histori['id_peminjaman_buku']."'");
            while($dt_buku=mysqli_fetch_array($qry_buku)){
                $buku_dipinjam.="<li>".$dt_buku['nama_buku']."</li>";
            }
            $buku_dipinjam.="</ol>";
            //menampilkan status sudah kembali atau belum
            $qry_cek_kembali=mysqli_query($connect,"select * from pengembalian_buku where id_peminjaman_buku = '".$dt_histori['id_peminjaman_buku']."'");
